...
class Events(models.Model):
    ...
    date_start = models.DateField('event start date', auto_now=True, auto_now_add=True)
    date_end = models.DateField('event end date', auto_now=False, auto_now_add=False)
    time_start = models.TimeField('event start time', auto_now=False, auto_now_add=True)
    ...
    launch_date = models.DateTimeField('date event launched', auto_now_add=False, auto_now=False)
    is_launched = models.BooleanField('event launched', default=False)

    def __str__(self):
        return '%s %s %s (%s)'.format(self.title, "@", self.venue, self.date_start)
